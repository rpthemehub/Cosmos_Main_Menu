...
class UserConfig {
 	...
 	</ label="Show cartridge art", help="Show cartridge or console art", options="cartridge, console, none", order=5 /> enable_gcartart="cartridge"; 
 	</ label="Artwork aspect ratio", help="Preserve aspect ratio of box and cartridge art", options="Yes,No", order=6 /> ratio="Yes";  
 	</ label="Enable animations", help="Enable box and cartridge art animations", options="Yes,No", order=7 /> enable_ganimate="Yes"; 
...
//adjust the values below for the static preview video snap
   ...
   local snapbg=null;
   snapbg = fe.add_image( "static.mp4", flx*0.21, fly*0.305, flw*0.31, flh*0.4);
...
surface_snap.set_pos(flx*0.212, fly*0.31, flw*0.293, flh*0.38);
...
if ( my_config["enable_gcartart"] == "cartridge" )
{
	local cartart = fe.add_artwork("cartart", flx*0.3, fly*0.65, flw*0.25, flh*0.35 );
...
if ( my_config["enable_gcartart"] == "console" )
{
    local cartart = fe.add_image("flyer2/[Emulator]", flx*0, fly*0, flw*1, flh*1 );
}


if ( my_config["enable_gcartart"] == "none" )
{
    local cartart = fe.add_image("", flx*0, fly*0, flw*1, flh*1 );
}
...
{
	local boxart = fe.add_artwork("boxart", flx*0, fly*0.30 flw*0.25, flh*0.7 );
...
if ( my_config["enable_ginfo"] == "Yes" )
  {
      ...
      //Title text info
      local textt = fe.add_text( "[Title]", flx*0.315, fly*0.955, flw*0.6, flh*0.026  );
      ...
      //Emulator text info
      local textt = fe.add_text( "[Emulator]", flx*0.75, fly*0.958, flw*0.2, flh*0.022  );
      textt.set_rgb( 225, 255, 255 );
      //textt.style = Style.Bold;
      textt.align = Align.Left;
      textt.rotation = 0;
      ...
      //display filter info
      local filter = fe.add_text( "[ListFilterName]: [ListEntry]-[ListSize]  [PlayedCount]", flx*0.7, fly*0.962, flw*0.3, flh*0.020 );
